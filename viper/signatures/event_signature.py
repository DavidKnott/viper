...
class EventSignature():
    def __init__(self, name, args, indexed_list, event_id, method_id, sig):
        self.name = name
        self.args = args
    ...
    def from_declaration(cls, code):
        ...
        sig = name + '(' + ','.join([canonicalize_type(arg.typ) for arg in args]) + ')'
        raw_id = sha3(bytes(sig, 'utf-8'))
        event_id = bytes_to_int(raw_id)
        method_id = fourbytes_to_int(raw_id[:4])
        return cls(name, args, indexed_list, event_id, method_id, sig)

    def to_abi_dict(self):
        return {
            "name": self.name,
            "inputs": [{"type": canonicalize_type(arg.typ), "name": arg.name, "indexed": self.indexed_list[pos]} for pos, arg in enumerate(self.args)] if self.args else [],
            "anonymous": False,
            "type": "event"
        }
